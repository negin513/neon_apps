...
def remove_outliers_manual (df, variable, min_thresh, max_thresh):

    df_new = df.copy()
    
    count = df_new[(df_new[variable] > max_thresh) | (df_new[variable] < min_thresh)].shape[0]
    df_new.loc[df_new[variable] > max_thresh, variable] = np.nan
    df_new.loc[df_new[variable] < min_thresh, variable] = np.nan
...
def get_neon_pkl_files(pkl_dir, neon_site, years):
    pkl_names = []
    for year in years:
        print (join(pkl_dir, neon_site+'_'+year+'df_all.pkl'))
...
def get_data (df_all, var, freq, this_site):
    ...
    df_this=df_all[df_all['site']==this_site]
